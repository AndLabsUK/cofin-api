...
func (g *Generator) Continue(ctx context.Context, company *models.Company, document *models.Document, chunks []string, message string) (string, error) {
	input := []schema.ChatMessage{
		schema.SystemChatMessage{
			Text: fmt.Sprintf(`
			You are COFIN AI, an Artificial Intelligence built purposely to analyse financial filings of publicly traded companies and answer questions based on them. You have access to 10-Q and 10-K filings.
	...
	input = append(input, schema.SystemChatMessage{
		Text: fmt.Sprintf(`
